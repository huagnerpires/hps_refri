...
class _TelaCustomWidgetState extends State<TelaCustomWidget> {
  ...
  Future<void> _verificarPermissao() async {
    try {
      ...
      final deviceId = await _tcGetDeviceId();
      FFAppState().nomedouser = deviceId;

      bool permissao = false;

      // 1) Buscar por email no ID_DO_CELULAR (padrão login6)
      if (email.isNotEmpty) {
        final snap = await FirebaseFirestore.instance
            .collection('AREA_RESTRITA')
            .where('ID_DO_CELULAR', isEqualTo: email)
            .limit(1)
            .get();
        if (snap.docs.isNotEmpty) {
          final d = snap.docs.first.data();
          permissao = d['permissao'] == true || d['PERMISSAO'] == true;
          if (mounted)
            setState(() {
      ...
      // 2) Buscar por deviceId no DEVICE_ID
        ...
        child: Center(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            FutureBuilder<String>(
              future: _tcGetDeviceId(),
              builder: (ctx, snap) => SelectableText(
                'Email logado: ${currentUserEmail}\nDevice ID: ${snap.data ?? "carregando..."}',
